...
function makeAutoN(type, name = -1, options = {}) {
    let output = exports.dereference(type);
    output.GUNS = [], output.TURRETS = [], output.CONTROLLERS = null;

    let
        number = Array.isArray(type) ? type.length : (options.number ? options.number : 2),
        size = options.size ? options.size : 11,
        xOffset = options.xOffset ? options.xOffset : 8,
        arc = options.swivel ? 360 : (options.arc ? options.arc : 230 - 10 * number),
        angleOffset = options.angleOffset ? options.angleOffset : 0,
        autospin = options.autospin == false ? false : true,
        typeString = type.
        swivel = options.swivel ? true : false;

    for (let i = 0; i < number; i++) {
        output.TURRETS.push({
            POSITION: [size, xOffset, 0, i * 360 / number + angleOffset, arc, swivel ? 1 : 0],
            PROPERTIES: {
                TYPE: Array.isArray(type) ? type[i] : type,
            }
        })
    }

    if (autospin) output.FACING_TYPE = "autospin";
    output.LABEL = name ? name : `${type.LABEL}-${number}`;
    output.DANGER = options.danger ? options.danger : (type.DANGER ? type.DANGER + number - 1 : 3);
    return output;
}
